// Логика для страницы лекций по программированию
...
let playlists = [
  {
    id: 1,
    title: 'Основы Python',
    description: 'Введение в язык программирования Python для начинающих.',
    icon: '📚',
    videos: []
  },
  {
    ...
    description: 'Полный курс по алгоритмам: сортировки, поиск, деревья, графы.',
    icon: '🎯',
    videos: []
  },
  {
  ...
  if (playlistForm) {
    playlistForm.addEventListener('submit', async (e) => {
      ...
      for (const url of videoUrls) {
        ...
        if (videoId) {
          // Используем стандартные превью YouTube
          videos.push({
            id: videoId,
            title: `Видео ${videos.length + 1}`, // Можно улучшить через API
...
function createVideoCard(title, desc, duration, src, type, playlistName = null) {
  ...
  // Кнопка для перехода на YouTube (для плейлистов)
...
function createArticleCard(title, category, author, excerpt, content) {
  ...
  // Также откроем по клику на заголовок или карточку
  const titleEl = card.querySelector('h3');
  [titleEl, card].forEach(el => {
    if (el) {
      el.addEventListener('click', (e) => {
        // Избежать двойного срабатывания
...
function openArticleView({ title, author, category, content }) {
  ...
  contentEl.textContent = content; // сохраняем переносы строк
